...
def main():
    ...
    mc = ModelCheckpoint(
        'best_model_TFRNN.keras', monitor='val_loss', mode='min', verbose=1, save_best_only=True,
    )
    history = model.fit(
        X_train_tfidf, y_train, epochs=100, batch_size=32, validation_split=0.2, callbacks=[es, mc],
    )
